...
#
# Database backup & restore utilities for SocioPredict
# Usage:
#   ./SocioPredict backup --save
#   ./SocioPredict backup --list
#   ./SocioPredict backup --latest
#   ./SocioPredict backup --restore </path/to/backup.dump.gz>
#   ./SocioPredict backup --restore-latest
#   ./SocioPredict backup --inspect </path/to/backup.dump.gz>
#   ./SocioPredict backup --help
#
...
#
# Notes:
# - Must be called via ./SocioPredict (enforced by CALLED_FROM_SOCIOPREDICT guard)
# - Uses .env for DB container/name/user/pass/ports
# - Writes backups to a directory PARALLEL to repo root: ../backups/
# - Filenames: sociopredict_backup_${APP_ENV}_${YYYYmmdd_HHMMSS}.dump.gz + .sha256
...
# .env is already sourced by the SocioPredict script before calling this file
...
need_container_running() {
  if ! container_running; then
    ...
    echo "Start it first: ./SocioPredict up"
    exit 1
  fi
}
...
latest_backup_file() {
  ls -1t "$BACKUP_ROOT"/sociopredict_backup_"${APP_ENV}"_*.dump.gz 2>/dev/null | head -n 1 || true
}

print_usage() {
  cat <<EOF
Usage: ./SocioPredict backup [--save | --list | --latest | --restore <file> | --restore-latest | --inspect <file> | --help]
...
Backups:
  sociopredict_backup_${APP_ENV}_YYYYmmdd_HHMMSS.dump.gz
  sociopredict_backup_${APP_ENV}_YYYYmmdd_HHMMSS.dump.gz.sha256
EOF
}
...
do_save() {
  ...
  ts="$(timestamp)"
  file="$BACKUP_ROOT/sociopredict_backup_${APP_ENV}_${ts}.dump.gz"
...
do_list() {
  ...
  ls -1t "$BACKUP_ROOT"/sociopredict_backup_"${APP_ENV}"_*.dump.gz 2>/dev/null || echo "(none found)"
}
