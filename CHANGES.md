...
* removed all compose scripts. They will be handled by the sociopredict script.
* removed all exec scripts. They will be handled by ./scripts/exec.sh script.
* removed start file.
* added script sociopredict that will handle all logic.
...
3. Make sure SocioPredict file is executable
```
chmod +x SocioPredict
```
...
5. Run SocioPredict Script
```
sudo ./SocioPredict install
```
...
* Database Name: the name for the database - Optional (defaults to sociopredict_db)
...
```
sudo ./SocioPredict up
```

9. And stop it with:
```
sudo ./SocioPredict down
```

## Logic Behind The Script:
* Each time you run the script (SocioPredict) first of all it checks if docker compose is installed or not. It only supports docker compose and not docker-compose since docker-compose has stopped receiving security updates for about a year now.
...
* Add more options to the SocioPredict script.
* Add a help section to the SocioPredict script to offer help messages regarding its usage.
