...
import (
	...
	"sociopredict/models"
	"sociopredict/models/modelstesting"
)
...
<div class="flex flex-col sm:flex-row items-center mb-8">
  <img src="/HomePageLogo.png" alt="SocioPredict Logo" class="w-24 h-24 sm:w-32 sm:h-32 mb-4 sm:mb-0 sm:mr-6" />
  <div class="flex flex-col justify-center h-full text-center sm:text-left">
    <h1 class="text-3xl sm:text-4xl font-bold text-custom-gray-light mb-2">SocioPredict</h1>
    <p class="text-lg text-custom-gray-light">An alpha project powered by SocioPredict's open-source prediction market platform.</p>
  </div>
</div>
...
<div class="space-y-8">
  <section class="bg-gray-800 rounded-lg p-6 shadow-lg">
	...
	if !strings.Contains(content.HTML, "SocioPredict") {
...
func TestSeedHomepage_FallbackContent(t *testing.T) {
	...
	if !strings.Contains(content.HTML, "Welcome to SocioPredict") &&
		!strings.Contains(content.HTML, "SocioPredict") {
