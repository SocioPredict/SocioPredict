...
const environments = {
  ...
  staging: {
    API_BASE_URL: 'https://api-staging.sociopredict.com/api',
    APP_ENV: 'staging',
    DEBUG: true,
    ENABLE_DEVTOOLS: true,
    LOG_LEVEL: 'info',
  ...
  production: {
    API_BASE_URL: 'https://api.sociopredict.com/api',
    APP_ENV: 'production',
    DEBUG: false,
...
// .env.staging
REACT_APP_API_BASE_URL=https://api-staging.sociopredict.com/api
...
// .env.production
REACT_APP_API_BASE_URL=https://api.sociopredict.com/api
...
function getRuntimeCaching(mode) {
  ...
  return [
    {
      urlPattern: /^https:\/\/api\.sociopredict\.com\/api\/markets/,
...
jobs:
  deploy-staging:
    ...
    - name: Run smoke tests
      working-directory: ./frontend
      run: npm run test:smoke -- --baseURL=https://staging.sociopredict.com
  ...
  deploy-production:
    ...
    - name: Create deployment backup
      run: |
        ...
        aws s3 sync s3://${{ secrets.PRODUCTION_S3_BUCKET }} s3://sociopredict-backups/frontend/$timestamp/
    ...
    - name: Blue-Green Deployment
      run: |
        ...
        # Run health checks
        curl -f https://blue.sociopredict.com/health || exit 1
        ...
        # Final health check
        curl -f https://sociopredict.com/health || exit 1
        
    - name: Run production smoke tests
      working-directory: ./frontend
      run: npm run test:smoke -- --baseURL=https://sociopredict.com
  ...
  rollback:
    ...
    - name: Restore from backup
      run: |
        ...
        aws s3 sync s3://sociopredict-backups/frontend/${{ github.event.inputs.backup_timestamp }}/ s3://${{ secrets.PRODUCTION_S3_BUCKET }} --delete
...
terraform {
  ...
  backend "s3" {
    bucket = "sociopredict-terraform-state"
...
resource "aws_s3_bucket" "frontend" {
  bucket = "sociopredict-frontend-${var.environment}"
}
...
resource "aws_wafv2_web_acl" "frontend" {
  name  = "sociopredict-frontend-${var.environment}"
  scope = "CLOUDFRONT"
  ...
  visibility_config {
    cloudwatch_metrics_enabled = true
    metric_name                = "sociopredict-frontend-${var.environment}"
    sampled_requests_enabled   = true
  }
}
