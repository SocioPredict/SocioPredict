...
__SP_ROOT_DIR="${SOCIOPREDICT_ROOT:-$(cd "${__SP_LOCAL_DIR}/.." && pwd)}"
...
# Guard: only run via ./SocioPredict
...
init_env() {
  ...
  if grep -q '^BACKEND_IMAGE_NAME=' "${__SP_ROOT_DIR}/.env"; then
    "${SED_INPLACE[@]}" "s|^BACKEND_IMAGE_NAME=.*|BACKEND_IMAGE_NAME=ghcr.io/YOUR_USERNAME/sociopredict-backend:latest|" "${__SP_ROOT_DIR}/.env"
  else
    printf "\nBACKEND_IMAGE_NAME=ghcr.io/YOUR_USERNAME/sociopredict-backend:latest\n" >> "${__SP_ROOT_DIR}/.env"
  fi
  if grep -q '^FRONTEND_IMAGE_NAME=' "${__SP_ROOT_DIR}/.env"; then
    "${SED_INPLACE[@]}" "s|^FRONTEND_IMAGE_NAME=.*|FRONTEND_IMAGE_NAME=ghcr.io/YOUR_USERNAME/sociopredict-frontend:latest|" "${__SP_ROOT_DIR}/.env"
  else
    printf "\nFRONTEND_IMAGE_NAME=ghcr.io/YOUR_USERNAME/sociopredict-frontend:latest\n" >> "${__SP_ROOT_DIR}/.env"
  fi
