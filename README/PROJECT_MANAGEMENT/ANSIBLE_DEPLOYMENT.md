...
Project Plan: SocioPredict Droplet Automation & Maintenance
Goal
Automate deployment and maintenance of the SocioPredict stack (from github.com/YOUR_USERNAME/sociopredict) on a DigitalOcean droplet, ensuring:
...
Clone/update the repo into /opt/sociopredict.
...
Optionally trigger a full ./SocioPredict up deployment.
...
Clone sociopredict into /opt/sociopredict (branch configurable).
...
Phase 3: Deployment (Optional, Separate or Combined)
 Build local images (sociopredict-backend, sociopredict-frontend) from specified branch.

 Run ./SocioPredict install and ./SocioPredict up.
...
./SocioPredict up starts the stack cleanly.

Project Plan 1: Get Ansible Running for SocioPredict
Goal
Set up Ansible so we can manage the SocioPredict droplet from our local machine (or a CI/CD runner) and handle deployments systematically.
...
Edit
---
- name: Initial SocioPredict setup
  hosts: sociopredict_droplet
  become: true
  tasks:
    - name: Ensure Git, Docker, and Compose are installed
      apt:
        ...
        state: present
        update_cache: yes
    - name: Clone SocioPredict repo
      git:
        repo: "https://github.com/YOUR_USERNAME/sociopredict.git"
        dest: "/opt/sociopredict"
        version: main
        force: yes
Verify Access
...
Confirm repo exists at /opt/sociopredict.
...
Goal
Deploy SocioPredict automatically to DigitalOcean whenever a branch is pushed or merged (e.g., to main or staging).
  ...
  build-deploy:
    ...
    steps:
      ...
      - name: Build Docker images
        run: |
          docker build -t sociopredict-backend ./backend
          docker build -t sociopredict-frontend ./frontend

      - name: Push images (optional)
        if: env.DOCKER_REGISTRY != ''
        run: |
          echo "${{ secrets.DOCKER_PASSWORD }}" | docker login ${{ secrets.DOCKER_REGISTRY }} -u ${{ secrets.DOCKER_USERNAME }} --password-stdin
          docker push ${{ secrets.DOCKER_REGISTRY }}/sociopredict-backend:latest
          docker push ${{ secrets.DOCKER_REGISTRY }}/sociopredict-frontend:latest
